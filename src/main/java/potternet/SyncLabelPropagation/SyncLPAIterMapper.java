...
public class SyncLPAIterMapper extends Mapper<LongWritable, Text, Text, Text> {
    ...
    public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
        ...
        for(String inNodeWithEdgeWeight : inNodeList) {
            ...
            context.write(
                new Text(inNodeName),
                new Text(nameKeyMap.get(outNodeName) + "#" + String.valueOf(inEdgeParamBeta * out2inWeight))
            );
            // Send pair: <out_node/name>, <<in_node_label>#<param_alpha * edge_weight>>
            context.write(
                new Text(outNodeName),
                new Text(nameKeyMap.get(inNodeName) + "#" + String.valueOf(outEdgeParamAlpha * out2inWeight))
            );
        }
